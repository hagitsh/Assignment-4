...
class MMCC:
    ...
    def run(self, num_features):
        initial_filter = np.apply_along_axis(self.correntropy1, 0, self.X.values)
        filter = np.argpartition(initial_filter, -num_features)[-num_features:]
        self.X = self.X.iloc[:, filter]
        return np.apply_along_axis(self.correntropy, 0, self.X.values)
...
class MMCCModel:
    ...
    def run_mmcc(self, params):
        ...
        return np.argpartition(mmcc.run(self.num_features), -self.num_features)[-self.num_features:]
    ...
    def select_features(self, X, y, num_features=100):
        ...
        results = fmin_cg(self.eval, [(1000, 3000), (0.01, 0.03)])
        print(results)
